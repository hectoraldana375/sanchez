...
@section('title', 'Reparaciones Clutch')

@section('content')
<div class="container mx-auto p-4">
    <!-- Título centrado -->
    <div class="flex justify-center items-center relative mb-8">
        <h1 class="text-4xl font-bold text-gray-900 text-center">Reparaciones Clutch</h1>
    </div>
    ...
    <div class="flex justify-center mb-8">
        <a href="{{ route('reconstructora') }}" class="px-6 py-3 bg-green-500 text-white font-bold rounded-lg shadow-md border border-black hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-400">Regresar</a>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($productos as $producto)
            <div class="bg-white p-4 rounded-lg shadow-md flex flex-col justify-between">
                <div class="relative w-full h-48 overflow-hidden rounded-lg mb-4">
                    <img src="{{ $producto->imagen }}" alt="{{ $producto->titulo }}" class="absolute inset-0 w-full h-full object-cover">
                </div>
                <h2 class="text-xl font-semibold mb-2">{{ $producto->titulo }}</h2>
                <p class="text-gray-700 mb-4 flex-grow">{{ $producto->descripcion }}</p>
                <p class="text-lg font-bold text-center">${{ number_format($producto->precio, 2) }}</p>
            </div>
        @endforeach
    </div>
</div>
